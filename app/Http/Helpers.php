...
if (!function_exists('discount_in_percentage')) {
    ...
    {
        $base = home_base_price($product->unit_price, false);
                        ...
                        foreach ($carts as $key => $cartItem) {
                            ...
                            foreach ($coupon_details as $key => $coupon_detail) {
                                if ($coupon_detail->product_id == $cartItem['product_id']) {
                                    if ($coupon->discount_type == 'percent') {
                                        $coupon_discount += (cart_product_price($cartItem, $product, false, false) * $coupon->discount / 100) * 1;
                                    } elseif ($coupon->discount_type == 'amount') {
                                        $coupon_discount += $coupon->discount * 1;
                                    }
                                }
                            }
                        }
...
if (!function_exists('home_base_price')) {
    function home_base_price($price, $formatted = true)
    {
