...
class ProductController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        if($request->categories == "" && $request->brands == "" && $request->colors == "" && $request->attrs == "" && $request->min == "" && $request->max == "" && $request->name == ""){
            ...
            return $products = DB::table('products')->join('users', 'users.id', '=', 'products.user_id')->join('uploads', 'uploads.id', '=', 'products.thumbnail_img')->join('shops', 'products.user_id', '=', 'shops.user_id')->join('brands', 'brands.id', '=', 'products.brand_id')->select('products.*', 'users.username', 'uploads.file_name', 'shops.id as shop_id', 'shops.logo', 'brands.name as brand_name')->paginate(50);
